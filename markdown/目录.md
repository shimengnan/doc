...
- AtomicInteger
  ...
  addAndGet	先加然后获取加后的值

  getAndAdd	先获取,后加

  getAndIncrement	  先获取后加一

  incrementAndGet 	先加一后获取

  decrementAndGet	先减一后获取

  getAndDecrement	先获取后减一

- AtomicLong

  与 AtomicInteger API 基本一致

- AtomicReference

  对引用的原子性修改, 引用变量的赋值本身没有并发问题.AtomicReference 的引入是为了用一种无锁方式操作共享资源,而不是通过悲观锁的方式.
...
- AtomicReferenceFieldUpdater
  ...
  一个基于反射的工具类,能对指定类,指定的volatile 引用字段进行原子更新(非private)

- AtomicStampedReference 与 AtomicMarkReference

  用于解决AtomicReference 中的ABA 问题.
...
- Striped64 与 LongAdder
  ...
  数据`striping` 就是把逻辑上连续的数据分为多个段,使这一序列的段存储在不同的设备上.通过把段分散到多个设备上可以增加并发访问性,从而提升总体的吞吐量.

  Striped64 是在1.8 版本引入的.采用了类似分段锁的机制,内部数据结构一个long 类型的 base 变量,和持有long 类型变量的Cell 数组.单线程情况下,仅需要对base 做加减计数,当遇到多线程,计算hash 值,找到对应的Cell,用cas 来修改cell 中的值,最后统计结果值即为 base 与 所有Cell 中的值.

  https://coderbee.net/index.php/concurrent/20140518/931

  http://www.manongjc.com/article/105666.html

##### 1.3.2.3 阻塞队列- BlockingQueue
...
- SynchronousQueue
  ...
  同步阻塞队列,每次put 必须等待一个take,反之亦然.同步队列没有任何内部容量.

  适合与传递性设计.支持公平策略,默认不公平.
...
##### 1.3.2.4 ConcurrentHashMap

##### 1.3.2.5 多线程同步通信类

- Semphore

  计数信号量 由一个指定数量的许可初始化,调用 `acquire()` 许可数量-1,调用 `release()` 许可数量+1,

  主要用于保护代码防止超过N个线程进入 或 在两个线程间发送信号. 

- Exchanger

  两个线程可以互相交换对象的 `会和点`,用于两个线程 通过 `exchanger.exchange()` 交换对象

- CyclicBarrier

  等待栅栏,用于在多线程场景下,当指定数量的线程抵达栅栏 await 方法后,继续执行后续业务.

  支持构造方法中传入runnable 对象,指定到达栅栏时的内容

- CountDownLatch

  允许一个或多个线程等待一系列指令操作完成.初始化 count,调用countDown 方法,count--.

  若count 不为0 调用await 方法阻塞.为0 时 await 执行.

##### 1.3.2.6 ThreadPool 

##### 1.3.2.7 ForkJoinPool
